...
(*Day 3, answer2 tests*)
let group_by_threee    = assert_equal [['i';'h';'g'];['f';'e';'d'];['c';'b';'a']] (group_by_three ['a';'b';'c';'d';'e';'f';'g';'h';'i']) ~printer:print_lst_lst

let intersection_of_lists = assert_equal 'a' (intersections [['a';'b';'c'];['z';'a';'y'];['v';'n';'a']]) ~printer:Char.escaped

let day_three_2_tests = "test suite for day3, answer 2" >:::[
    "check firstn" >:: (fun _ -> assert_equal ([['c';'b';'a'];['d';'e']])
        (firstn 3 [] ['a';'b';'c';'d';'e']) ~printer:print_lst_lst );
