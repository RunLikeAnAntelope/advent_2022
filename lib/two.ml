...
let apply_strat str =
  determine_strat_score (String.get str 0) (String.get str 2)
...
let read_strategy filename =
  let channel = open_in filename in
  read_strat channel []

let calculate_answer1 file = read_strategy file |> acc_strat

let calculate_answer2 file =
  read_strategy file |> List.map apply_strat
  |> List.fold_left (fun acc x -> acc + x) 0

let answer1 file =
  "1: Strategy Score = " ^ string_of_int (calculate_answer1 file)

let answer2 file =
  "2: New Strategy Score = " ^ string_of_int (calculate_answer2 file)
